...
class ProcessMap(widgets.DOMWidget):
    ...
    def get_duplicate_edges_and_sources_and_sinks(self, filtered_log):
        ...
        for case_id, traces in events:
            ...
            node_pairs = [
                (traces[self.activity_field].iloc[i], traces[self.activity_field].iloc[i + 1])
    ...
    def on_filter_change(self, change={}):
        ...
        case_ids = concat([self.activities.get_group(trace).index.to_series() for trace in traces_to_include])
        ...
        duplicate_edges, sources, sinks = self.get_duplicate_edges_and_sources_and_sinks(filtered_log)
